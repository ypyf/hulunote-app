...
pub fn AllDatabasesPage() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ...
        if authed && app_state.0.databases.get().is_empty() {
            load_databases();
        }
    });
    ...
    let submit_create_database = move |_| {
    ...
    let open_rename_dialog = move |db: &Database| {
        rename_db_id.set(Some(db.id.clone()));
        rename_value.set(db.name.clone());
        rename_error.set(None);
        rename_open.set(true);
    };

    let submit_rename_database = move |_| {
    ...
    let open_delete_dialog = move |db: &Database| {
        delete_db_id.set(Some(db.id.clone()));
        delete_db_name.set(db.name.clone());
        delete_confirm.set(String::new());
        delete_error.set(None);
        delete_open.set(true);
    };

    let submit_delete_database = move |_| {
        ...
        spawn_local(async move {
            match api_client.delete_database_by_id(&id).await {
                Ok(_) => {
                    // Reload list and keep selection valid.
                    let mut c = app_state.0.api_client.get_untracked();
                    if let Ok(dbs) = c.get_database_list().await {
                        app_state.0.databases.set(dbs.clone());

                        // If current selected DB is gone, pick the first.
                        let current = app_state.0.current_database_id.get_untracked();
                        let still_exists = current
                            .as_deref()
                            .map(|cid| dbs.iter().any(|d| d.id == cid))
                            .unwrap_or(false);
                        ...
                        if !still_exists {
                            if let Some(first) = dbs.first() {
                                ...
                                if let Some(storage) =
                                    web_sys::window().and_then(|w| w.local_storage().ok().flatten())
                                {
                                    let _ = storage.set_item(CURRENT_DB_KEY, &first.id);
                                }
                            } else {
                                ...
                                if let Some(storage) =
                                    web_sys::window().and_then(|w| w.local_storage().ok().flatten())
                                {
                                    let _ = storage.set_item(CURRENT_DB_KEY, "");
                                }
                            }
                        }
                    }
        ...
        <div class="space-y-4">
            <div class="flex items-start justify-between gap-3">
                <div class="space-y-1">
                    ...
                    <p class="text-xs text-muted-foreground">"Manage databases here."</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button
                        ...
                        attr:disabled=move || db_loading.get()
                    >
                        {move || if db_loading.get() { "Loading..." } else { "Refresh" }}
                    </Button>
                </div>
            </div>
            ...
            <Show
                ...
                fallback=move || view! {
                    <div class="rounded-md border border-border bg-muted p-4 text-sm text-muted-foreground">
                        {move || if db_loading.get() { "Loading databases…" } else { "No databases." }}
                    </div>
                }
            >
                <div class="space-y-2">
                    {move || {
                        app_state
                            ...
                            .map(|db| {
                                ...
                                // Separate clones for different callbacks.
                                let db_for_click = db.clone();
                                let db_for_rename = db.clone();
                                let db_for_delete = db.clone();
                                let id_for_open = id.clone();

                                view! {
                                    <div class="group flex items-center justify-between gap-2 rounded-md border border-border bg-background px-3 py-2">
                                        <a
                                            href=format!("/db/{}", id_href)
                                            class="min-w-0 flex-1"
                                            on:click=move |_| {
                                                // keep recent db fresh
                                                write_recent_db(&db_for_click.id, &db_for_click.name);
                                            }
                                        >
                                            <div class="truncate text-sm font-medium">{name}</div>
                                            <div class="truncate text-xs text-muted-foreground">{id}</div>
                                        ...
                                        <div class="hidden items-center gap-2 group-hover:flex">
                                            <Button
                                                ...
                                                on:click=move |ev| {
                                                    ev.stop_propagation();
                                                    open_rename_dialog(&db_for_rename);
                                                }
                                            >
                                                "Rename"
                                            </Button>
                                            <Button
                                                ...
                                                on:click=move |ev| {
                                                    ev.stop_propagation();
                                                    open_delete_dialog(&db_for_delete);
                                                }
                                            >
                                                "Delete"
                                            </Button>
                                            <Button
                                                ...
                                                on:click=move |ev| {
                                                    ...
                                                    navigate.with_value(|nav| {
                                                        nav(&format!("/db/{}", id_for_open), Default::default());
                                                    });
                                                }
                                            >
                                                "Open"
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
                    <div class="w-full max-w-sm rounded-md border border-border bg-background p-4 shadow-lg">
                        <div class="mb-3 space-y-1">
                            ...
                            <div class="text-xs text-muted-foreground">"Create a new database (max 5)."</div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-end gap-2 pt-2">
                                ...
                                <Button
                                    size=ButtonSize::Sm
                                    attr:disabled=move || create_loading.get()
                                    on:click=submit_create_database
                                >
                                    <span class="inline-flex items-center gap-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-end gap-2 pt-2">
                                ...
                                <Button
                                    size=ButtonSize::Sm
                                    attr:disabled=move || rename_loading.get()
                                    on:click=submit_rename_database
                                >
                                    <span class="inline-flex items-center gap-2">
                    ...
                    <div class="w-full max-w-sm rounded-md border border-border bg-background p-4 shadow-lg">
                        <div class="mb-3 space-y-1">
                            ...
                            <div class="text-xs text-muted-foreground">
                                "Type the database name to confirm. This is a soft delete on the backend."
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-end gap-2 pt-2">
                                ...
                                <Button
                                    ...
                                    attr:disabled=move || delete_loading.get()
                                    on:click=submit_delete_database
                                >
                                    <span class="inline-flex items-center gap-2">
